...
export type CommentableType = 'achievement.comment' | 'trigger.ticket.comment' | 'forum-topic-comment' | 'game.comment' | 'game-hash.comment' | 'game-modification.comment' | 'leaderboard.comment' | 'achievement-set-claim.comment' | 'user.comment' | 'user-activity.comment' | 'user-moderation.comment';
export type GameActivitySnapshotType = 'trending' | 'popular';
export type MessageThreadTemplateKind = 'achievement-issue' | 'manual-unlock' | 'misclassification' | 'unwelcome-concept' | 'writing-error';
...
items: TItems[];
links: {firstPageUrl:string | null;lastPageUrl:string | null;previousPageUrl:string | null;nextPageUrl:string | null;};
};
export type Role = {
id: number;
name: string;
...
preferences?: {isGloballyOptedOutOfSubsets:boolean;prefersAbsoluteDates:boolean;shouldAlwaysBypassContentWarnings:boolean;};
